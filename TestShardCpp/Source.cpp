...
int main1(int argc, char **argv) {
	srand(0);
	Agent **agentList = new Agent*[1024];
...
//Common.h
void toRGBf(const float h, const float s, const float v, rgbf* rgb)
{
	/*
	RGBType rgb;
	if(!h  && !s)
...
void drawCircle(const float data, float *out) {
...
void display()
{
	...
	for (int i = 0; i < size; i++) {
		...
		int canvasX = i % window_width;
		int canvasY = i / window_width;
		if (canvasX * canvasX + canvasY * canvasY < 10000) {
			pixels[i * 3] = 1;
			pixels[i * 3 + 1] = 1;
			pixels[i * 3 + 2] = 1;
		} else {
